...
export async function UpdatePromptToNotion({auth, databaseId}, {id,title, desc, category, rawPrompt, sampleImgLink}) {
    let token = ""
    if (auth.startsWith("Bearer ")) {
        token = auth.substring(7)
    } else {
...
export async function GetPromptsList({auth, databaseId,pageSize, startCursor}) {
    let token = ""
    if (auth.startsWith("Bearer ")) {
        token = auth.substring(7)
    } else {
    ...
    const cursor = startCursor ? startCursor : undefined
    return notion.databases.query({
        database_id: databaseId,
        filter: {
            property: "type",
